...
export function SiteFooter(){return <footer className="bg-navy text-navy-foreground"><div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
 <div className="grid gap-12 lg:grid-cols-[1fr_1.5fr]"><div className="max-w-sm"><Link href="/" className="flex items-center gap-2.5" aria-label="Five Nines Logistics home"><FiveNinesMark className="size-7"/><span className="text-sm font-semibold">FIVE NINES LOGISTICS</span></Link><p className="mt-4 text-sm leading-relaxed text-navy-foreground/70">Houston-based 3PL built for mission-critical contractors, plant maintenance teams, data centers, and international shippers—from expedited freight and truckload to heavy haul, drayage, crane and rigging, ocean, and warehousing. Backed by trusted capacity, 24/7 service, and nationwide and global reach.</p><div className="mt-5 flex flex-col gap-1 text-sm text-navy-foreground/70"><a href={site.phoneHref}>{site.phone}</a><a href={`mailto:${site.dispatchEmail}`}>{site.dispatchEmail}</a></div></div>
