...
export function SiteFooter(){return <footer className="bg-navy text-navy-foreground"><div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
 <div className="grid gap-12 lg:grid-cols-[1fr_1.5fr]"><div className="max-w-sm"><Link href="/" className="flex items-center gap-2.5" aria-label="Five Nines Logistics home"><FiveNinesMark className="size-7"/><span className="text-sm font-semibold">FIVE NINES LOGISTICS</span></Link><p className="mt-4 text-sm leading-relaxed text-navy-foreground/70">3PL brokerage and operating coordination for critical, scheduled, and specialty freight. Established carriers and nationwide and global partners extend the right capability for each move.</p><div className="mt-5 flex flex-col gap-1 text-sm text-navy-foreground/70"><a href={site.phoneHref}>{site.phone}</a><a href={`mailto:${site.dispatchEmail}`}>{site.dispatchEmail}</a></div></div>
 ...
 <div className="mt-12 border-t border-white/10 pt-6"><p className="max-w-4xl text-xs leading-relaxed text-navy-foreground/55">Five Nines Logistics is a freight-facing 3PL and brokerage brand operating as an agent of Primary Freight LLC, MC# 841023. Transportation is performed by independently authorized motor carriers selected for each shipment. Carrier equipment and third-party or leased warehouse facilities are not represented as Five Nines-owned assets.</p><div className="mt-4 flex flex-col gap-2 font-mono text-[10px] uppercase tracking-wider text-navy-foreground/55 sm:flex-row sm:justify-between"><span>© {new Date().getFullYear()} Five Nines Logistics · Houston, Texas</span><span>Authority and insurance documents available by request</span></div></div>
 </div></footer>}
